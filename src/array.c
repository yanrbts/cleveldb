...
void ldb_array_grow(ldb_array_t *z, size_t zn) {
    if (zn > z->alloc) {
        z->items = (uint64_t *)ldb_realloc(z->items, zn * sizeof(uint64_t));
        z->alloc = zn;
    }
}
...
static void ldb_swap(uint64_t *items, int i, int j) {
...
static int ldb_partition(uint64_t *items, int lo, int hi, int (*cmp)(uint64_t, uint64_t)) {
    uint64_t pivot = items[(hi + lo) >> 1];
    int i = lo - 1;
    int j = hi + 1;
...
static void ldb_qsort(uint64_t *items, int lo, int hi, int (*cmp)(uint64_t, uint64_t)) {
    if (lo >= 0 && hi >= 0 && lo < hi) {
